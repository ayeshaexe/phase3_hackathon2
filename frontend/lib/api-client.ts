...
class ApiClient {
  ...
  // Chat methods
  async sendMessage(userId: string, message: string, conversationId?: number): Promise<ApiResponse> {
  ...
  async getUserConversations(userId: string, limit: number = 20, offset: number = 0): Promise<ApiResponse> {
    return this.request(`/api/${userId}/conversations?limit=${limit}&offset=${offset}`);
  }

  async getConversation(userId: string, conversationId: number): Promise<ApiResponse> {
